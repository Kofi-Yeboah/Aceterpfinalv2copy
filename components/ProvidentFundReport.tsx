...
export function ProvidentFundReport() {
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedDepartment, setSelectedDepartment] = useState("All Departments");
  const [showDepartmentDropdown, setShowDepartmentDropdown] = useState(false);

  const providentData = [
    { id: "1", employeeId: "EMP-001", employeeName: "John Smith", department: "Engineering", basicSalary: 8500, employeeContribution: 510, employerContribution: 765, totalContribution: 1275, cumulativeBalance: 28500 },
    { id: "2", employeeId: "EMP-002", employeeName: "Sarah Johnson", department: "Sales", basicSalary: 7200, employeeContribution: 432, employerContribution: 648, totalContribution: 1080, cumulativeBalance: 24000 },
    { id: "3", employeeId: "EMP-003", employeeName: "Michael Brown", department: "Marketing", basicSalary: 6800, employeeContribution: 408, employerContribution: 612, totalContribution: 1020, cumulativeBalance: 22600 },
    { id: "4", employeeId: "EMP-004", employeeName: "Emily Davis", department: "Operations", basicSalary: 7500, employeeContribution: 450, employerContribution: 675, totalContribution: 1125, cumulativeBalance: 26200 },
    { id: "5", employeeId: "EMP-005", employeeName: "David Wilson", department: "HR", basicSalary: 6500, employeeContribution: 390, employerContribution: 585, totalContribution: 975, cumulativeBalance: 21500 },
    { id: "6", employeeId: "EMP-006", employeeName: "Lisa Martinez", department: "Finance", basicSalary: 6200, employeeContribution: 372, employerContribution: 558, totalContribution: 930, cumulativeBalance: 20400 },
    { id: "7", employeeId: "EMP-007", employeeName: "Tom Anderson", department: "Engineering", basicSalary: 7000, employeeContribution: 420, employerContribution: 630, totalContribution: 1050, cumulativeBalance: 23800 },
    { id: "8", employeeId: "EMP-008", employeeName: "Rachel Green", department: "Sales", basicSalary: 5500, employeeContribution: 330, employerContribution: 495, totalContribution: 825, cumulativeBalance: 18200 },
  ];

  const filteredData = providentData.filter((record) => {
  ...
  const totalCumulativeBalance = filteredData.reduce((sum, record) => sum + record.cumulativeBalance, 0);

  return (
    <div className="h-full flex flex-col bg-slate-50 overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-200 bg-white flex justify-between items-center">
        <h1 className="text-2xl font-semibold text-slate-900">Provident Fund Report</h1>
        ...
        <table className="w-full">
          <thead>
            <tr style={{ backgroundColor: "#0B01D0" }}>
              ...
              <th className="text-left px-4 py-3 text-white text-[12px] font-semibold border-b border-slate-100">Employee Contribution (6%)</th>
              <th className="text-left px-4 py-3 text-white text-[12px] font-semibold border-b border-slate-100">Employer Contribution (9%)</th>
              <th className="text-left px-4 py-3 text-white text-[12px] font-semibold border-b border-slate-100">Total Contribution</th>
              <th className="text-left px-4 py-3 text-white text-[12px] font-semibold border-b border-slate-100">Cumulative Balance</th>
            </tr>
          </thead>
          <tbody>
            {filteredData.map((record, index) => (
              <tr key={record.id} className={`border-b border-slate-100 ${index % 2 === 0 ? "bg-white" : "bg-slate-50"}`}>
                ...
                <td className="px-4 py-4"><p className="text-[12px] font-semibold text-green-600">{formatCurrency(record.cumulativeBalance)}</p></td>
              </tr>
            ))}
            {filteredData.length > 0 && (
              <tr style={{ backgroundColor: "#0B01D0" }}>
                ...
                <td className="px-4 py-4"><p className="text-[12px] font-semibold text-white">{formatCurrency(totalCumulativeBalance)}</p></td>
              </tr>
            )}
          </tbody>
        </table>
