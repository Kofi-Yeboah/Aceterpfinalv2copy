...
import { addProjectFolder, addVaultDocument } from "../lib/documentVaultStore";
import type { FileType } from "../lib/documentVaultStore";
    ...
    <>
      {selectedProject ? (
        ...
        <AddProjectForm
          ...
          onSave={(projectData) => {
            // Auto-create a document vault folder for the new project
            const folderId = `p${Date.now()}`;
            const createdDate = new Date().toLocaleDateString("en-US", { month: "short", day: "2-digit", year: "numeric" });
            addProjectFolder({
              id: folderId,
              name: projectData.projectName,
              createdDate,
              program: projectData.program || undefined,
              origin: "auto",
            });

            // Push uploaded files from the project creation flow into the vault
            if (projectData.uploadedFiles && projectData.uploadedFiles.length > 0) {
              for (const uf of projectData.uploadedFiles) {
                const ext = uf.name.split(".").pop()?.toUpperCase() ?? "PDF";
                const safeExt: FileType = (["PDF","DOCX","XLSX","PPTX","DOC","PNG","JPG"].includes(ext) ? ext : "PDF") as FileType;
                addVaultDocument({
                  id: `d${Date.now()}-${uf.id}`,
                  name: uf.name,
                  fileType: safeExt,
                  size: uf.size,
                  uploadedBy: uf.uploadedBy || "PMO Admin",
                  uploadedDate: uf.uploadedAt || createdDate,
                  projectId: folderId,
                  projectName: projectData.projectName,
                  department: "Projects",
                  category: "Project Charter",
                  version: `v${uf.version}.0`,
                });
              }
            }
