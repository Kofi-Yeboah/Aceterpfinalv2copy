...
export function ProjectBudgets() {
  ...
  const types = ["All Types", ...Array.from(new Set(projectBudgetData.map(p => p.projectType)))];
  const years = ["All Years", ...Array.from(new Set(projectBudgetData.map(p => p.startDate.split(" ")[1])))];

  const filteredData = projectBudgetData.filter(p => {
    if (selectedType !== "All Types" && p.projectType !== selectedType) return false;
    if (selectedYear !== "All Years" && !p.startDate.includes(selectedYear)) return false;
    if (selectedStatus !== "All Statuses" && p.status !== selectedStatus) return false;
    if (searchQuery) {
      const q = searchQuery.toLowerCase();
      return p.projectName.toLowerCase().includes(q) || p.projectManager.toLowerCase().includes(q);
    }
    return true;
  });
      ...
      <div className="px-6 py-4 bg-white border-b border-slate-200">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select
            ...
            className="px-4 py-2 border border-slate-200 rounded-lg text-sm text-slate-700 bg-white min-w-[180px] focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
          ...
          <select
            ...
            className="px-4 py-2 border border-slate-200 rounded-lg text-sm text-slate-700 bg-white min-w-[120px] focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
          ...
          <select
            ...
            className="px-4 py-2 border border-slate-200 rounded-lg text-sm text-slate-700 bg-white min-w-[160px] focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
