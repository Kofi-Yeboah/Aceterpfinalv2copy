...
const RATING_SCALE = [
  { range: "70 & Below", label: "Under Performed", badgeColor: "border-red-200 text-red-700 bg-red-50" },
  { range: "71 – 80", label: "Target Not Met", badgeColor: "border-orange-200 text-orange-700 bg-orange-50" },
  { range: "81 – 90", label: "Target Met", badgeColor: "border-blue-200 text-blue-700 bg-blue-50" },
  { range: "91 – 95", label: "Target Exceeded", badgeColor: "border-teal-200 text-teal-700 bg-teal-50" },
  { range: "96 – 100", label: "Outstanding", badgeColor: "border-green-200 text-green-700 bg-green-50" },
];
...
function makeDefaultObjectives(): ScorecardObjective[] {
  ...
  const labels: Record<string, string[]> = {
    sd: ["Develop annual talent management framework", "Rollout HR ERP system", "Design & develop manpower resource plan"],
    ip: ["Automate performance management system", "Recruitment process automation", "Mitigating circumstances for HR risk register"],
    sp: ["Embedding 5C culture across the business", "Establish monthly staff allocation 1-2-1 meetings"],
    ld: ["Design and implement HR dashboard with all metrics"],
  };
  const targets: Record<string, string[]> = {
    sd: ["Developed, approved & embedded for N-1 and HODs", "100% Rollout & embedded", "Resource plan embedded for each department and reviewed quarterly"],
    ip: ["ACET performance management system implemented", "Internal ATS implemented and embedded", "Updated ACET register with mitigation"],
    sp: ["Assessment complete and clear plan designed for each department", "Date confirmed for one-to-one and recurrence diarised"],
    ld: ["Dashboard rollout and implemented"],
  };
  ...
  const dueDates: Record<string, string[]> = {
    sd: ["2025-10-15", "2026-06-15", "2025-03-15"],
    ip: ["2026-06-15", "2026-06-15", "2026-12-15"],
    sp: ["2025-04-15", "2025-03-15"],
    ld: ["2026-06-15"],
  };
  for (const cfg of PERSPECTIVE_CONFIG) {
    const count = labels[cfg.prefix]?.length || 1;
    for (let i = 0; i < count; i++) {
      objectives.push({
        id: `${cfg.prefix}${i + 1}`,
        label: labels[cfg.prefix]?.[i] || "",
        target: targets[cfg.prefix]?.[i] || "",
        weight: weights[cfg.prefix]?.[i] || 5,
        dueDate: dueDates[cfg.prefix]?.[i] || "",
        midYear: emptyReview(),
        endYear: emptyReview(),
      });
    }
...
const defaultBehaviors: BehaviorItem[] = [
  { id: "b1", label: "Pioneering", expectation: "Champion new initiatives that anticipate future organisational needs", rating: 0, weight: 5 },
  { id: "b2", label: "Respect", expectation: "Foster a culture where all staff feel valued, heard, and fairly treated", rating: 0, weight: 5 },
  { id: "b3", label: "Integrity", expectation: "Ensure transparent procedures in all processes", rating: 0, weight: 5 },
  { id: "b4", label: "Collaboration", expectation: "Support teams to build cohesive and high-performing units", rating: 0, weight: 5 },
  { id: "b5", label: "Excellence", expectation: "Hold team members accountable for delivering exceptional service", rating: 0, weight: 5 },
];
...
function getRatingLabel(score: number | null): { label: string; color: string; bg: string } {
  ...
  if (score <= 70) return { label: "Under Performed", color: "text-red-700", bg: "bg-red-50" };
  if (score <= 80) return { label: "Target Not Met", color: "text-orange-700", bg: "bg-orange-50" };
  if (score <= 90) return { label: "Target Met", color: "text-blue-700", bg: "bg-blue-50" };
  if (score <= 95) return { label: "Target Exceeded", color: "text-teal-700", bg: "bg-teal-50" };
...
export function PerformanceManagementScreen({ viewOnly = false }: { viewOnly?: boolean } = {}) {
  ...
  const updateObjectiveField = (id: string, field: "label" | "target" | "dueDate", value: string) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const updateBehaviorWeight = (id: string, value: number) => {
    setFormData((prev) => ({
      ...prev,
      behaviors: prev.behaviors.map((b) => (b.id === id ? { ...b, weight: value } : b)),
    }));
  };

  const totalBehaviorWeight = formData.behaviors.reduce((s, b) => s + b.weight, 0);
  const behaviorWeightRemaining = 25 - totalBehaviorWeight;
  const behaviorWeightOk = totalBehaviorWeight === 25;
  ...
  if (view === "detail" && detailRecord) {
    ...
    const renderPeriodBreakdown = (period: PeriodScores, periodLabel: string) => {
      ...
      const behTotal = period.behaviors.reduce((s, b) => s + b.weight, 0);
      ...
      return (
        <div className="space-y-4 py-1">
          ...
          {/* Balanced Scorecard — collapsible perspectives */}
          {PERSPECTIVE_CONFIG.map((group) => {
            ...
            const sectionKey = `${periodLabel}-${group.key}`;
            ...
            return (
              <div key={group.key} className="border border-slate-200 rounded-lg overflow-hidden">
                ...
                >
                  ...
                  <span className={`${group.color} text-[12px] flex items-center gap-1.5`}>
                  ...
                  <span className="text-[12px] text-slate-500 shrink-0">{contribution.toFixed(1)} / {maxContribution}</span>
                </button>

                {isExpanded && (
                  <div className="border-t border-slate-100 divide-y divide-slate-50">
                    {objs.map((obj) => {
                      const score = calcFinalScore(obj);
                      return (
                        <div key={obj.id} className="px-5 py-3 pl-11">
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              <p className="text-[13px] text-slate-800">{obj.label || "—"}</p>
                              <p className="text-[11px] text-slate-400 mt-0.5">Target: {obj.target || "—"}</p>
                            </div>
                            <div className="flex items-center gap-4 shrink-0">
                              {obj.dueDate && (
                                <span className="text-[11px] text-slate-400">{new Date(obj.dueDate).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}</span>
                              )}
                              <span className="text-[11px] text-slate-400">{obj.weight}%</span>
                              <span className="text-[12px] text-slate-700 font-medium w-10 text-right">{score.toFixed(1)}</span>
                            </div>
                          </div>
                        </div>
                      );
          ...
          {/* Behavioral Assessment — collapsible */}
          {(() => {
            const behKey = `${periodLabel}-behavioral`;
            const isBehExpanded = !!expandedSections[behKey];
            const behContribution = period.behaviors.reduce((s, b) => s + (b.rating / 5) * b.weight, 0);

            return (
              <div className="border border-slate-200 rounded-lg overflow-hidden">
                <button
                  onClick={() => toggleSection(behKey)}
                  className="w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition-colors"
                >
                  <ChevronRight className={`w-4 h-4 text-slate-400 transition-transform ${isBehExpanded ? "rotate-90" : ""}`} />
                  <span className="text-indigo-600 text-[12px] flex items-center gap-1.5">
                    <Lightbulb className="w-4 h-4" /> Behavioral Assessment
                  </span>
                  <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden mx-3">
                    <div className="h-full rounded-full bg-indigo-500 transition-all" style={{ width: `${behTotal ? (behContribution / behTotal) * 100 : 0}%` }} />
                  </div>
                  <span className="text-[12px] text-slate-500 shrink-0">{behContribution.toFixed(1)} / {behTotal}</span>
                </button>

                {isBehExpanded && (
                  <div className="border-t border-slate-100 divide-y divide-slate-50">
                    {period.behaviors.map((b) => (
                      <div key={b.id} className="px-5 py-3 pl-11">
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1 min-w-0">
                            <p className="text-[13px] text-slate-800 font-medium">{b.label}</p>
                          ...
                          <div className="flex items-center gap-4 shrink-0">
                            ...
                            <div className="flex items-center gap-0.5">
                              {Array.from({ length: 5 }).map((_, i) => (
                                <Star key={i} className={`w-3.5 h-3.5 ${i < b.rating ? "text-amber-400 fill-amber-400" : "text-slate-200 fill-slate-200"}`} />
                              ))}
                            </div>
                            <span className="text-[12px] text-slate-700 font-medium w-10 text-right">{((b.rating / 5) * b.weight).toFixed(1)}</span>
                          </div>
                        </div>
                      </div>
                    ))}
        ...
        <div className="flex-1 overflow-auto p-6">
                      ...
                      <table className="w-full">
                        <thead style={{ backgroundColor: "#0B01D0" }}>
                          <tr className="bg-slate-50 border-b border-slate-100">
                            <th className="text-left px-3 py-2 text-[11px] text-white font-semibold" rowSpan={2} style={{ width: "18%" }}>Objective</th>
                            <th className="text-left px-3 py-2 text-[11px] text-white font-semibold" rowSpan={2} style={{ width: "15%" }}>Target</th>
                            <th className="text-center px-3 py-2 text-[11px] text-white font-semibold" rowSpan={2} style={{ width: "6%" }}>Weight&nbsp;%</th>
                            <th className="text-center px-3 py-2 text-[11px] text-white font-semibold" rowSpan={2} style={{ width: "9%" }}>Due Date</th>
                            <th className={`text-center px-2 py-1.5 text-[11px] border-b border-slate-200 ${activePeriodColor}`} colSpan={3} style={{ width: "38%" }}>
                              {activePeriodLabel}
                            </th>
                            <th className="text-center px-3 py-2 text-[11px] text-white font-semibold" rowSpan={2} style={{ width: "7%" }}>Final Score</th>
                          </tr>
                          ...
                          {groupObjs.map((obj, idx) => {
                            ...
                            return (
                              <tr key={obj.id} className={`border-b border-slate-50 ${idx % 2 !== 0 ? "bg-slate-50/40" : ""}`}>
                                ...
                                {/* Due Date */}
                                <td className="px-3 py-2">
                                  <div className="text-[12px] text-slate-600 text-center whitespace-nowrap">
                                    {obj.dueDate ? new Date(obj.dueDate).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }) : "—"}
                                  </div>
                                </td>
          ...
          {activeFormTab === 3 && (
            <div className="max-w-3xl mx-auto space-y-4">
              ...
              {/* Behavioral weight tracker */}
              <div className={`flex items-center gap-3 px-4 py-3 rounded-lg border text-[13px] ${
                behaviorWeightOk ? "bg-green-50 border-green-200 text-green-700"
                  : behaviorWeightRemaining < 0 ? "bg-red-50 border-red-200 text-red-700"
                  : "bg-amber-50 border-amber-200 text-amber-700"
              }`}>
                {behaviorWeightOk ? <CheckCircle2 className="w-4 h-4 shrink-0" /> : <AlertCircle className="w-4 h-4 shrink-0" />}
                <span>
                  Behavioral weighting: <strong>{totalBehaviorWeight}% / 25%</strong>
                  {!behaviorWeightOk && behaviorWeightRemaining > 0 && ` — ${behaviorWeightRemaining}% remaining`}
                  {!behaviorWeightOk && behaviorWeightRemaining < 0 && ` — ${Math.abs(behaviorWeightRemaining)}% over`}
                  {behaviorWeightOk && " — Target reached"}
                </span>
                <div className="ml-auto flex-shrink-0 w-32 h-2 bg-white/60 rounded-full overflow-hidden border border-current/20">
                  <div className={`h-full rounded-full transition-all ${behaviorWeightOk ? "bg-green-500" : behaviorWeightRemaining < 0 ? "bg-red-500" : "bg-amber-400"}`} style={{ width: `${Math.min((totalBehaviorWeight / 25) * 100, 100)}%` }} />
                </div>
              </div>
              ...
              <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                <table className="w-full">
                  <thead style={{ backgroundColor: "#0B01D0" }}>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      <th className="text-left px-4 py-3 text-[11px] text-white font-semibold" style={{ width: "15%" }}>Core Behavior</th>
                      <th className="text-left px-4 py-3 text-[11px] text-white font-semibold" style={{ width: "30%" }}>Employee Objective</th>
                      <th className="text-center px-4 py-3 text-[11px] text-white font-semibold" style={{ width: "20%" }}>Rating (1–5)</th>
                      <th className="text-center px-4 py-3 text-[11px] text-white font-semibold" style={{ width: "10%" }}>Weight %</th>
                      <th className="text-right px-4 py-3 text-[11px] text-white font-semibold" style={{ width: "10%" }}>Contribution</th>
                    </tr>
                  </thead>
                  <tbody>
                    {formData.behaviors.map((b, idx) => (
                      <tr key={b.id} className={`border-b border-slate-100 ${idx % 2 !== 0 ? "bg-slate-50/40" : ""}`}>
                        ...
                        <td className="px-4 py-3">
                          <p className="text-[12px] text-slate-600">{b.expectation || "—"}</p>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex justify-center">
                    ...
                    <tr className="bg-indigo-50 border-t border-indigo-100">
                      <td className="px-4 py-2 text-[12px] text-indigo-700" colSpan={2}>Behavioral Total</td>
                      <td />
                      <td className="text-center px-4 py-2 text-[12px] text-indigo-700">{totalBehaviorWeight}%</td>
                      <td className="text-right px-4 py-2 text-[12px] text-indigo-700">
                        {formData.behaviors.reduce((s, b) => s + (b.rating / 5) * b.weight, 0).toFixed(1)}
                      </td>
                    </tr>
                  </tbody>
                </table>
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                      <div className="h-full rounded-full bg-indigo-500 transition-all" style={{ width: `${totalBehaviorWeight ? (formData.behaviors.reduce((s, b) => s + (b.rating / 5) * b.weight, 0) / totalBehaviorWeight) * 100 : 0}%` }} />
                    </div>
                    <span className="text-[12px] text-slate-600 w-32 text-right shrink-0 whitespace-nowrap">
                      {formData.behaviors.reduce((s, b) => s + (b.rating / 5) * b.weight, 0).toFixed(1)} / {totalBehaviorWeight}
                    </span>
                  </div>
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-200 bg-white flex items-center justify-between">
        <h1 className="text-[20px] text-slate-900">ACET Scorecard</h1>
        <div className="flex items-center gap-2">
          <span className={`px-3 py-1.5 rounded-full text-[12px] flex items-center gap-1.5 ${
            isMidYearActive ? "bg-blue-50 text-blue-700 border border-blue-200" : "bg-purple-50 text-purple-700 border border-purple-200"
          }`}>
            <Lock className="w-3 h-3" />
            {isMidYearActive ? "Mid-Year Review Active" : "End-of-Year Review Active"}
          </span>
        </div>
      </div>
      ...
      <div className="px-6 py-3 bg-white border-b border-slate-200 flex items-center gap-3">
        ...
        <div className="relative">
          <button onClick={() => { setShowYearDropdown((p) => !p); setShowDeptDropdown(false); setShowStatusDropdown(false); }} className="flex items-center gap-2 px-4 py-2 border border-slate-200 rounded-lg text-[13px] text-slate-700 hover:bg-slate-50 bg-white transition-colors min-w-[100px] justify-between">
        ...
        <div className="relative">
          <button onClick={() => { setShowDeptDropdown((p) => !p); setShowYearDropdown(false); setShowStatusDropdown(false); }} className="flex items-center gap-2 px-4 py-2 border border-slate-200 rounded-lg text-[13px] text-slate-700 hover:bg-slate-50 bg-white transition-colors min-w-[180px] justify-between">
        ...
        <div className="relative">
          <button onClick={() => { setShowStatusDropdown((p) => !p); setShowYearDropdown(false); setShowDeptDropdown(false); }} className="flex items-center gap-2 px-4 py-2 border border-slate-200 rounded-lg text-[13px] text-slate-700 hover:bg-slate-50 bg-white transition-colors min-w-[150px] justify-between">
            <span>{statusTab === "All" ? "All Statuses" : statusTab}</span><ChevronDown className="w-4 h-4" />
          </button>
          {showStatusDropdown && (
            <div className="absolute top-full left-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg z-20 py-1 min-w-[170px]">
              {STATUS_TABS.map((s) => (
                <button key={s} onClick={() => { setStatusTab(s); setShowStatusDropdown(false); }} className={`w-full text-left px-4 py-2 text-[13px] hover:bg-slate-50 flex items-center justify-between ${s === statusTab ? "text-blue-600" : "text-slate-700"}`}>
                  <span>{s === "All" ? "All Statuses" : s}</span>
                  <span className="text-[11px] text-slate-400">{statusCounts[s]}</span>
                </button>
              ))}
            </div>
          )}
