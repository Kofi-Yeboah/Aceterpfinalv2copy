...
export function OperationalBudgets() {
  ...
  const departments = ["All Departments", ...Array.from(new Set(budgetData.map(b => b.department)))];
  const categories = ["All Categories", ...Array.from(new Set(budgetData.map(b => b.category)))];
  const years = ["All Years", ...Array.from(new Set(budgetData.map(b => b.budgetYear)))];

  const filteredData = budgetData.filter(b => {
    if (selectedDepartment !== "All Departments" && b.department !== selectedDepartment) return false;
    if (selectedCategory !== "All Categories" && b.category !== selectedCategory) return false;
    if (selectedYear !== "All Years" && b.budgetYear !== selectedYear) return false;
    if (selectedStatus !== "All Statuses" && b.status !== selectedStatus) return false;
    if (searchQuery) {
      const q = searchQuery.toLowerCase();
      return b.department.toLowerCase().includes(q) || b.category.toLowerCase().includes(q);
    }
    return true;
  });
