...
```
Procurement Plan → Requisition → Sourcing → Award → Contract → Deliverables → Invoice → Payment → Close-Out → Supplier Rating
```
...
4. [Flow 3 — Supplier onboarding and lifecycle](#flow-3--supplier-onboarding-and-lifecycle)
...
9. [Flow 8 — Bid submission and the sealed register](#flow-8--bid-submission-and-the-sealed-register)
...
Coordinator, Supervisor, Audit, Supplier. Each holds a set of capabilities (`plan.reviewFinance`,
...
| Supplier document expiry | 24h | — | 14 days |
...
which stops the chase. Notifications reach their recipients in the bell menu and by email; the
reminder engine runs behind the screens rather than having a menu of its own.
...
field-level `changeHistory` with old and new values; suppliers carry `statusHistory`. Every entry
...
flowchart TD
    ...
    H -- Reject + reason --> E
    H -- Approve --> I[APPROVED — live and usable]
...
An approved plan item, live and available to Flow 2.
...
## Flow 3 — Supplier onboarding and lifecycle

**Requirement:** BR002 · **Screens:** Supplier Management, Approvals → Supplier Registrations · **Code:** `lib/supplierStore.ts`, `components/Suppliers.tsx`, `components/ProcurementApprovalQueues.tsx`

```mermaid
flowchart TD
    A1[Procurement captures supplier details] --> B[Screening]
...
One record per supplier, created internally by
Procurement. Both call `registerSupplier()`, which issues the next `SUP-NNNN` identifier (firms
...
### Automatic screening — `getSupplierFlags()`
...
An Active supplier selectable in Sourcing, whose rating and contract history are visible in both
...
flowchart TD
    ...
    B --> C[2. Advertisement<br/>website + external channels]
...
| 2 | Advertisement | Procurement | `publishTender()` records the advertisement and its channels (ACET website, newspapers, donor portals) with dates and references. This is what makes the tender visible in Flow 8. |
...
| 4 | Invitation | Procurement | Selects shortlisted bidders from the supplier database — gated by `checkSourcingEligibility()` — or registers a new supplier inline. Specifies whether technical, financial or both proposals are required. For individual consultants this may instead be an interview invitation. |
| 5 | Electronic submission | Suppliers | See Flow 8. Sealed on arrival. |
...
- No supplier blocked by the eligibility gate can be invited or awarded.
...
shortlist only (`invitedSupplierIds`), so only those suppliers may be recorded against it.
...
Single-source procurement. No advertisement; the invitation step becomes "Select Supplier" with a
...
Steps: Solicitation documents → Select supplier → Submission (optional) → Bid opening →
...
## Flow 8 — Bid submission and the sealed register

**Requirement:** BR003 · **Screen:** Procurement → Sourcing → case → Bid Opening · **Code:** `lib/tenderPortalStore.ts`, `components/SourcingCaseDetail.tsx`

```mermaid
flowchart TD
    A[Offer arrives from supplier] --> B[Procurement records it against the tender]
    B --> C[Supplier, amount, date and documents captured]
    C --> D[System issues SUB-YYYY-NNNN<br/>+ ACK-YYYY-NNNN receipt]
    D --> E[Submission SEALED<br/>timestamp recorded]
    E --> F{Tender still open?}
    F -- Yes --> G[Entry may be withdrawn and replaced]
    F -- No --> H[Locked]
    H --> I[Procurement opens technical envelope]
    I --> J[Technical evaluation]
    J --> K[Procurement opens financial envelope]
    K --> L[Financial evaluation]
```
...
- **Audit trail** — timestamp, who recorded it, document count and opener are all recorded. `getSubmissionTimestampReport()` produces the timestamps report, exportable to Excel or PDF.
- **Restricted tenders** — a supplier not on `invitedSupplierIds` cannot be recorded against a Limited or Direct tender.
- **Eligibility** — only Active suppliers may be recorded; suspended and blacklisted ones are refused with an explanation.
- **No duplicate envelopes** — one technical and one financial envelope per supplier per tender; replacing requires withdrawing first, and only while the tender is open.
- **Individual consultants** may have a proposed daily or monthly rate captured where the tender requests it; the accepted rate is written to their rate history on award.

Everything else that touches suppliers is handled internally: registration and compliance documents
in Supplier Management (Flow 3), registration approval in Approvals → Supplier Registrations, and
invoice capture against a contract in Procurement → Invoices (Flow 12).
...
```mermaid
flowchart TD
    A[Evaluation complete] --> B[Select winning supplier]
    ...
    G --> I[Supplier contract history updated]
    H --> J{PO value >= $10,000?}
    J -- Yes --> K[COO signature]
    J -- No --> L[Procurement Head signature]
    K & L --> M[PO dispatched to supplier]
...
`recordContractAward()` writes the award into the supplier's contract history and totals.
...
1. **Identity** — title, auto-derived contract number, supplier, category, method, department, owner, funding source and budget line.
...
**Contracts arriving from a Sourcing award** are created as a shell (supplier, value, dates). The
...
```mermaid
flowchart TD
    A1[Invoice arrives by email or on paper] --> B[SUBMITTED]
...
(`getContractFinancials()`). Any reviewer may **query** an invoice back to the supplier with a
...
## Flow 14 — Supplier performance evaluation
...
On submission, `addPerformanceEvaluation()` pushes the scorecard into the supplier profile via
`recordSupplierEvaluation()`, which:

- appends it to the supplier's evaluation history, visible in both Contract Management and Supplier Management;
...
- **automatically flags any supplier scoring below 5**, after which engagement requires Senior Management approval through the eligibility gate in Flow 3.
...
**Requirement:** BR007 · **Screen:** Procurement → Reporting & Analytics · **Code:** `components/ProcurementReports.tsx`, `lib/exportUtils.ts`

Five reports, all computed from live store data, all searchable and all exportable to
**Excel (.xls)**, **PDF** and **CSV**. Each has the same shape — four summary cards across the
top, one table beneath them:

| Report | What it shows |
|---|---|
| Procurement Plan | Every plan activity with approval status, planned value, method and target date. Cards: activities and how many are approved, planned value, awaiting review, delayed. |
| Purchase Requisitions | Every requisition with stage, value, department and days in stage. Cards: requisitions, awaiting approval, approved value, converted to sourcing. |
| Contracts | The portfolio — supplier, value, spend to date, end date, derived risk. Cards: active contracts, contract value, invoiced to date, expiring within 90 days. |
| Invoices & Payments | Every invoice with contract, supplier, amount and position in the approval chain. Cards: invoices and total value, awaiting approval, paid, outstanding. |
| Supplier Performance | The register with rating, contracts awarded, status and document position. Cards: suppliers and how many are active, average rating, flagged or blocked, document issues. |
...
| Supplier flagged or poorly performing | Eligibility gate | Engagement permitted only with Senior Management approval |
| Supplier suspended or blacklisted | Eligibility gate | Blocked outright from invitation and award |
| Invoice disputed | Query with reason | Returns to the supplier; resubmission restarts the chain |
...
| BR002 Supplier management | 3 | `lib/supplierStore.ts`, `components/Suppliers.tsx` |
...
| BR003 Sealed bid register | 8 | `lib/tenderPortalStore.ts`, `components/SourcingCaseDetail.tsx` |
...
| BR004 Performance measurement | 14 | `lib/contractStore.ts` → `lib/supplierStore.ts` |
| BR004 Close-out | 15 | `lib/contractStore.ts` |
| BR007 Reporting & analytics | 16 | `components/ProcurementReports.tsx`, `lib/exportUtils.ts` |
...
- A blacklisted supplier is blocked from sourcing; a flagged one is allowed only with management approval; a debarred name is caught by sanctions screening at registration; a self-registration cannot be sourced until approved.
...
- A sub-5 evaluation automatically flags the supplier, and their next engagement then requires management approval — the loop from performance back to eligibility closes.
...
Purchase Orders, Invoices, Contract Management, Approvals, Reporting &
...
requires management approval before that supplier can be engaged. That is the control working, not a
