        ...
        <div className="flex-1 overflow-auto p-6">
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              ...
              <p className="text-[11px] text-slate-500 mt-2">{freshPR.category} — {freshPR.purchaseType}</p>
            </div>
          </div>
                ...
                <div key={i} className="flex items-start gap-3 px-3 py-2.5 bg-slate-50 rounded-lg">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <p className="text-[12px] font-medium text-slate-900">Step {entry.step} — {entry.role}</p>
          ...
          {freshPR.seniorMgmtApproval === "Pending" && (
            <div className="flex items-center gap-3">
              <button onClick={() => handleApprove(freshPR.id)} className="px-6 py-2.5 rounded-lg text-[13px] font-medium text-white bg-green-600 hover:bg-green-700 transition-colors flex items-center gap-2">
                <Check className="w-4 h-4" /> Grant Final Approval
              </button>
              <button onClick={() => setRejectModalOpen(freshPR.id)} className="px-6 py-2.5 rounded-lg text-[13px] font-medium text-white bg-red-600 hover:bg-red-700 transition-colors flex items-center gap-2">
        ...
        {rejectModalOpen && (
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl w-full max-w-md p-6">
              <h3 className="text-[15px] font-semibold text-slate-900 mb-3">Reject — Senior Management</h3>
              <p className="text-[12px] text-slate-500 mb-4">Provide reason for rejection:</p>
              <textarea value={rejectComment} onChange={(e) => setRejectComment(e.target.value)} rows={3} className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-[12px] text-slate-900 outline-none focus:border-red-400 resize-none" placeholder="e.g., Exceeds budget ceiling, requires board approval..." />
    ...
    <div className="flex flex-col h-full bg-slate-50">
      <div className="px-6 py-4 bg-white border-b border-slate-200 shrink-0">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[12px] text-slate-500 mt-1">Final approval for procurement requests exceeding $10,000 (Step 5)</p>
          </div>
      ...
      <div className="px-6 py-3 bg-white border-b border-slate-200 shrink-0">
        <div className="bg-slate-100 p-1 rounded-lg inline-flex gap-1">
          {tabs.map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-1.5 rounded-lg text-[12px] transition-colors min-w-[100px] flex items-center justify-center gap-1.5 ${activeTab === tab ? "bg-purple-700 text-white shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
              {tab}
      ...
      <div className="px-6 py-3 bg-white border-b border-slate-200 shrink-0">
        <div className="relative max-w-md">
          ...
          <input type="text" placeholder="Search by PR number, description, requester..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-4 py-2 text-[12px] border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent" />
        </div>
      </div>
        ...
        <table className="w-full">
          ...
          <tbody>
            {filteredRequests.length === 0 ? (
              <tr><td colSpan={8} className="text-center py-12 text-[13px] text-slate-400">No high-value procurement requests pending senior approval.</td></tr>
              ...
              <tr key={pr.id} className={`border-b border-slate-100 hover:bg-slate-50 transition-colors cursor-pointer ${index % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`} onClick={() => setSelectedRequest(pr)}>
                <td className="px-4 py-3"><span className="text-[12px] text-purple-700 font-medium">{pr.requisitionNumber}</span></td>
                <td className="px-4 py-3 text-[12px] text-slate-900 max-w-[220px] truncate">{pr.itemDescription}</td>
      ...
      {rejectModalOpen && !selectedRequest && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl w-full max-w-md p-6">
            <h3 className="text-[15px] font-semibold text-slate-900 mb-3">Reject — Senior Management</h3>
            <textarea value={rejectComment} onChange={(e) => setRejectComment(e.target.value)} rows={3} className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-[12px] text-slate-900 outline-none focus:border-red-400 resize-none" placeholder="Reason for rejection..." />
            <div className="flex items-center justify-end gap-3 mt-4">
              <button onClick={() => { setRejectModalOpen(null); setRejectComment(""); }} className="px-4 py-2 text-[12px] text-slate-600 hover:bg-slate-100 rounded-lg transition-colors">Cancel</button>
              <button onClick={() => handleReject(rejectModalOpen)} className="px-4 py-2 text-[12px] text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors font-medium">Confirm Rejection</button>
            </div>
