...
// This screen is the Department Head station (step 2) of the requisition
// workflow. Approving here routes the requisition on to the concurrent
// Procurement and Finance reviews.
const APPROVE_CAPABILITY: Capability = "pr.approveDept";
const QUEUE_ROLE = "Department Head" as const;
...
/**
 * Everything an approver must see before deciding: the audit exceptions
 * (emergency override), the cost/plan variance, method deviations, and the
 * history of any earlier rejection.
 */
      ...
      <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 flex items-start gap-3">
        ...
        <div>
          <p className="text-[12px] font-semibold text-amber-900">
            You are signed in as {user.name} ({user.roles.join(", ")}) and cannot action the Department Head queue.
          </p>
...
function toExportRow(pr: GeneratedPR, stationStatus: StationStatus | null) {
  return {
    ...
    stationStatus: stationStatus ?? "Not in queue",
...
const EXPORT_COLUMNS: ExportColumn<ExportRow>[] = [
  ...
  { key: "stationStatus", header: "Dept Head Decision" },
...
export function ProcurementRequestApproval() {
  ...
  const stationStatusOf = (pr: GeneratedPR): StationStatus | null => {
    ...
    if (pr.deptApproval === "Approved") return "Approved";
    if (pr.deptApproval === "Rejected") return "Rejected";
    return null;
  };

  // Drafts never reached this station, and withdrawn requisitions are no longer
  // actionable by anyone.
  const queue = allPRs.filter((pr) => stationStatusOf(pr) !== null && pr.overallApprovalStatus !== "Withdrawn");

  const filteredRequests = queue.filter((pr) => {
    ...
    const matchesSearch =
      ...
      pr.department.toLowerCase().includes(needle);
    return matchesTab && matchesSearch;
  });

  const tabCounts: Record<TabType, number> = {
    All: queue.length,
    Pending: queue.filter((pr) => stationStatusOf(pr) === "Pending").length,
    Approved: queue.filter((pr) => stationStatusOf(pr) === "Approved").length,
    Rejected: queue.filter((pr) => stationStatusOf(pr) === "Rejected").length,
  };
  ...
  const handleApprove = (id: string) => {
    ...
    approvePRDept(id, "", currentUser.name);
  ...
  const handleReject = (id: string) => {
    ...
    const result = rejectPRDept(id, reason, currentUser.name);
    if (!result.ok) {
  ...
  const handleExport = (format: "excel" | "pdf" | "csv") => {
    ...
    const title = "Department Head Requisition Approvals";
    const meta = {
      ...
      <div className="flex flex-col h-full bg-slate-50">
        ...
        <div className="px-6 py-4 bg-white border-b border-slate-200 flex items-center gap-4 shrink-0">
          ...
          <div className="flex-1">
            ...
            <p className="text-[12px] text-slate-500 mt-0.5">{freshPR.itemDescription}</p>
          </div>
          <span
        ...
        <div className="flex-1 overflow-auto p-6">
          ...
          <div className="grid grid-cols-5 gap-4 mb-6">
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              ...
              <p className="text-[18px] font-semibold text-slate-900">${freshPR.estimatedCost.toLocaleString()}</p>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              <div className="flex items-center gap-2 mb-2">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              ...
              <p className="text-[18px] font-semibold text-slate-900">{freshPR.category}</p>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-[10px] text-slate-400 mt-0.5">
                With {freshPR.currentResponsible ?? "Requesting Officer"}
              </p>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              <div className="flex items-center gap-2 mb-2">
          ...
          {/* Action Buttons — only while this station holds the requisition */}
          {actionable && (
            <div>
              {!canApprove && (
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      ...
      <div className="px-6 py-4 bg-white border-b border-slate-200 shrink-0">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-[18px] font-semibold text-slate-900">Procurement Request Approval</h1>
        ...
        stats={[
          {
            ...
            sub: "Requisitions held at Department Head",
          ...
          <tbody>
            {filteredRequests.length === 0 ? (
              ...
              filteredRequests.map((pr, index) => {
                const tabStatus = stationStatusOf(pr);
                const actionable = tabStatus === "Pending";
                return (
                  <tr
                    key={pr.id}
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-[12px] text-slate-900">{pr.requestedBy}</td>
                    <td className="px-4 py-3 text-[12px] text-slate-600">{pr.department}</td>
                    <td className="px-4 py-3 text-[12px] text-slate-900 text-right font-medium">
                      ${pr.estimatedCost.toLocaleString()}
                    </td>
                    <td className="px-4 py-3 text-[12px] text-slate-600">{pr.dateRequested}</td>
                    <td className="px-4 py-3">
