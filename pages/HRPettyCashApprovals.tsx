...
const mockData: PettyCashApproval[] = [
  { id: "1", requestId: "PC-2026-0041", employee: "Kofi Mensah", department: "Project Management", purpose: "Office stationery and printing supplies", category: "Office Supplies", amount: 250, currency: "GHS", requestDate: "Feb 25, 2026", retirementDue: null, status: "Pending", notes: "Needed urgently for upcoming board meeting preparation." },
  { id: "2", requestId: "PC-2026-0038", employee: "Abena Owusu", department: "Finance", purpose: "Staff meeting refreshments", category: "Meals & Refreshments", amount: 180, currency: "GHS", requestDate: "Feb 20, 2026", retirementDue: "Mar 06, 2026", status: "Approved", notes: "Monthly all-hands meeting refreshments for 25 staff." },
  { id: "3", requestId: "PC-2026-0035", employee: "Richard Antwi", department: "Operations", purpose: "Courier service for document delivery", category: "Logistics", amount: 120, currency: "GHS", requestDate: "Feb 17, 2026", retirementDue: "Mar 03, 2026", status: "Pending", notes: "Urgent document delivery to Kumasi regional office." },
  { id: "4", requestId: "PC-2026-0032", employee: "Nana Yaw", department: "HR", purpose: "Emergency plumbing repair materials", category: "Maintenance", amount: 450, currency: "GHS", requestDate: "Feb 14, 2026", retirementDue: null, status: "Approved", notes: "Plumbing issue in ground floor washroom." },
  { id: "5", requestId: "PC-2026-0029", employee: "Mercy Adjei", department: "Programs", purpose: "Fuel for generator during power outage", category: "Utilities", amount: 300, currency: "GHS", requestDate: "Feb 10, 2026", retirementDue: null, status: "Pending", notes: "Extended power outage required generator operation for 2 days." },
  { id: "6", requestId: "PC-2026-0026", employee: "Kwame Asante", department: "Engineering", purpose: "Transport for field visit to Kumasi office", category: "Transport", amount: 200, currency: "GHS", requestDate: "Feb 06, 2026", retirementDue: null, status: "Rejected", notes: "Rejected — use official vehicle instead." },
  { id: "7", requestId: "PC-2026-0023", employee: "Ama Serwaa", department: "Admin", purpose: "Kitchen supplies for office pantry", category: "Office Supplies", amount: 95, currency: "GHS", requestDate: "Feb 03, 2026", retirementDue: null, status: "Approved", notes: "Monthly restocking of kitchen essentials." },
  { id: "8", requestId: "PC-2026-0020", employee: "Desmond Tutu", department: "Programs", purpose: "Printing materials for stakeholder workshop", category: "Office Supplies", amount: 180, currency: "GHS", requestDate: "Jan 28, 2026", retirementDue: null, status: "Pending", notes: "Printing for 50-person workshop." },
];
...
export function HRPettyCashApprovals() {
  const [searchQuery, setSearchQuery] = useState("");
  const [statusFilter, setStatusFilter] = useState("All Statuses");
  const [data, setData] = useState(mockData);
  const [viewItem, setViewItem] = useState<PettyCashApproval | null>(null);
    ...
    <div className="h-full flex flex-col bg-slate-50 overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-200 bg-white shrink-0">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h1 className="text-2xl font-semibold text-slate-900">Petty Cash Approvals</h1>
            <p className="text-[12px] text-slate-500">Review and approve petty cash requests</p>
          </div>
            ...
            <tbody>
              {filtered.map((item, idx) => (
                <tr key={item.id} className={`border-b border-slate-100 hover:bg-slate-50 transition-colors ${idx % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}>
                  ...
                  <td className="px-6 py-3 text-[12px] text-slate-600 max-w-[200px] truncate">{item.purpose}</td>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={9} className="px-6 py-16 text-center"><p className="text-[13px] text-slate-400">No petty cash approvals found</p></td></tr>
              )}
            </tbody>
          </table>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setViewItem(null)}>
          <div className="bg-white rounded-2xl w-full max-w-lg mx-4 overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
              <h3 className="text-[16px] text-slate-900">Petty Cash Request Details</h3>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><p className="text-[10px] text-slate-400 uppercase tracking-wider mb-1">Retirement Due</p><p className="text-[13px] text-slate-900">{viewItem.retirementDue || "—"}</p></div>
      ...
      {rejectingId && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50" onClick={() => { setRejectingId(null); setRejectReason(""); }}>
          <div className="bg-white rounded-2xl w-full max-w-md mx-4 overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
              <h3 className="text-[15px] text-slate-900">Reject Petty Cash Request</h3>
...
export default HRPettyCashApprovals;